...
import time
import parl
from agent import MujocoAgent
from model import MujocoModel
...
def run_evaluate_episode(env, agent, is_render=False):
    obs = env.reset()
    total_reward = 0
    while True:
        batch_obs = np.expand_dims(obs, axis=0)
        ...
        while total_steps < args.train_total_steps:
            ...
            if total_steps // args.test_every_steps >= test_flag:
                while total_steps // args.test_every_steps >= test_flag:
                    test_flag += 1
                evaluate_reward = run_evaluate_episode(env, agent, is_render=False)
